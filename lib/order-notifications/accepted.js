...
, build: function( order, logger, options, callback ){
    ...
    scheduler.enqueue('delivery-service-order', moment().add('minutes', 3).toDate(), { orderId: order.id});
...
, build: function( order, logger, options, callback ){
    ...
    app.render( 'order-email/gb-asap-order', viewOptions, function( error, html ){
      if ( error ){
        return callback( error );
      }

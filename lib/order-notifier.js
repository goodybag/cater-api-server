...
module.exports.send = function( id, order, options, callback ){
  ...
  module.exports.getNotification( id, order, options, function( error, notification ){
    ...
    var tasks = {
      sendMail:     module.exports.getSendFn(notification, order)
...
module.exports.sendSms = function(data, callback) {
  utils.twilio.sendSms(job.data, function(error) {
    if ( error ) logger.error('Error sending sms', { error: error });
    callback(error);
  });
}

module.exports.getSendFn = function(notification, order) {
  var format = notification.format || 'email';
  if ( format === 'email' ) {
    return utils.sendMail2.bind(null, notification);
  } else if ( format === 'sms' ) {
    return module.exports.sendSms;
  } else if ( format === 'voice' ) {
    return module.exports.sendVoice;
  } else {

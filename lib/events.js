  ...
  function( order, previous ){
    ...
    order.getRestaurant( function( error ){
      ...
      if (order.attributes.restaurant.voice_phones) {
        logger.info("making call for order: ", order.attributes.id);
      ...
      if (order.attributes.restaurant.sms_phones) {
        logger.info("shortening url and sending sms for order", order.attributes.id);
        ...
        bitly.shorten(url, function(err, response) {
          if (err) logger.error('unable to shorten url, attempting to sms unshortend link', err);
...
var orderChanges = {
  ...
  function( order, oldDatetime ){
    ...
    logger.info( order.attributes.datetime, oldDatetime );

    order.getOrderItems( function( error, items ){
      if ( error ) return logger.error( 'Could not get order items', error );
...
var deliveryServices = {
  ...
  function notifyAcceptedOrder( order, previous ){
    ...
    order.getRestaurant( function( error ){
      ...
      logger.info('Submitted day', submittedDay);

...
var orderChangeStatusEvents = {
  'order-change:status:change': function(change, oldStatus) {
    var order; // TODO: get order.  don't know if we want to fetch it from the db here or pass it in from the change model
    return; // TODO: delete once everything is ready

    var oid = order.attributes.id;
    var changeStatusEmailMap = {
      submitted: {
        to: order.attributes.restaurant.email,
        template: 'change-email/change-submitted',
        subject: 'A Goodybag customer would like to change their order (#' + oid + ')'
      },
      canceled: {
        to: order.attributes.restaurant.email,
        template: 'change-email/change-canceled',
        subject: 'A Goodybag customer has canceled their change to order (#' + oid + ')'
      },
      accepted: {
        to: order.attributes.user.email,
        template: 'change-email/change-accepted',
        subject: 'Your requested change to Goodybag order (#oid) has been accepted'
      },
      denied: {
        to: config.emails.denied,
        template: 'change-email/change-denied',
        subject: 'Proposed change to order (#' + oid + ') has been denied'
      }
    };

    var info = changeStatusEmailMap[change.attributes.status];
    var context = {change: change.toJSON(), order: order.toJSON({review:true}), layout: 'email-layout'};
    var email = {
      to: info.to,
      from: config.email.orders,
      subject: info.subject
    };

    app.render(info.template, context, function(err, html) {
      if (err) return logger.events.error(err);
      utils.sendMail(utils.extend({html: html}, email), function(err) {
        if (err) return logger.events.error(err);
      });
    });
  }

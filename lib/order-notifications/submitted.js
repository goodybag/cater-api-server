...
notifier.register({
  ...
  , isAvailable: function( order ){ return !order.restaurant.has_contract; }
  , build: function( order, logger, options, callback) {
...
notifier.register({
  ...
  , name: 'Notify Jacob when promo code is used'
  , description: 'Send Jacob an email when promo code is used'
  , build: function( order, logger, options, callback ){
    var viewOptions = {
      layout: false
    , config: config
    ...
    var promoEmails = Object.keys(promoConfig).filter( function (k) {
      return promoConfig[k].promo_code === order.promo_code;
    }).map( function (k) {

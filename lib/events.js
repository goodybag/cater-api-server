...
var restaurantEmail = {
  ...
  function( order, status ){
    ...
    if ( ['submitted', 'canceled'].indexOf( statusText ) > -1 ){
      var view = {
      ...
      res.render( 'email-order-' + statusText, view, function( error, html ){
        if ( error ) return logger.events.error( error );
        ...
        , html:       html
        , subject: [
            statusText === 'canceled' ? '' : 'You have received a new '
          , 'Goodybag Order (#'
          , order.attributes.id
          , ')'
          , statusText === 'submitted' ? '' : ' has been canceled'
          ].join('')
        };
...
var jukebox = {
  'order:status:change':
  function( order, status ){
    if ( status.attributes.status !== 'accepted' ) return;

...
, build: function( order, logger, options, callback ){
    ...
    , subject:    [ '[REMINDER] Goodybag Order #'
                  ...
                  , ({
                    ...
                    , courier:  ' to be delivered by a courier'
                    , pickup:   ' to be picked up'
                    })[ order.type ]
                  , order.datetime || order.pickup_datetime
                    ? ' on ' + moment( 
                        order.type === 'delivery' ? order.datetime : order.pickup_datetime 
                      ).format('MM-DD-YYYY')
                    : ' tomorrow'
                  ].join('')
    };
...
, build: function( order, logger, options, callback ){
    ...
    , subject:    [ '[REMINDER] Goodybag Order #'
                  , order.id
                  , ({
                      delivery: ' to be delivered by you'
                    , courier:  ' to be picked up by a courier'
                    , pickup:   ' to be picked up by the customer'
                    })[ order.type ]
                  , order.datetime || order.pickup_datetime
                    ? ' on ' + moment( 
                        order.type === 'delivery' ? order.datetime : order.pickup_datetime
...
, build: function( order, logger, options, callback ){
    ...
    utils.bitly.shorten(url, function(err, response) {
      ...
      logger.info( 'Setting up SMS', sms );

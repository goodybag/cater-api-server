  ...
  function( id, order, options, callback ){
    ...
    try {
      notification.build.call( notification, order, _logger, options, function( error, email ){
        if ( error ){
          _logger.warn( 'Error running notification builder', {
            error: error
          });
        }
...
module.exports.send = function( id, order, options, callback ){
  ...
  module.exports.getNotification( id, order, options, function( error, notification ){
    ...
    var data = Object.assign( utils.omit( notification, 'html' ), {options} );

    return Promise.try(() => {
      return module.exports.sendNotification(id, notification);
    }).then(() => {
      return db.order_notifications.insertAsync({
        order_id: orderId
      , data
      , nid:      id
      });
      ...
      return db.order_notification_failures.insertAsync({
        order_id: orderId
      , data
      , error: JSON.stringify({name: err.name, message: err.message, stack: err.stack})
      , nid: id
      }).catch((dbErr) => {
        slogger.error( 'Error saving notfiication failure', {
          error: dbErr
        , email: data
        });
      }).throw(err);
...
module.exports.sendNotification = function(id, notification) {
  var format = module.exports.defs[id].format;

  if ( format === 'email' ) {
    return Promise.fromNode((callback) => {
      utils.sendMail2(notification, callback);
    });
  } else if ( format === 'sms' ) {
    return module.exports.sendSms(notification);
  } else if ( format === 'voice' ) {
    return module.exports.makeCall(notification);
  } else {
    return Promise.reject(new Error(`Unknown notification format: "${format}"`));
  }
};

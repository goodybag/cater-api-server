...
, build: function( order, logger, options, callback ){
    ...
    , subject:    [ '[REMINDER] Goodybag Order #'
                  ...
                  , emailSubject.datetime
                    ? ' on ' + moment( emailSubject.datetime ).format('MM-DD-YYYY')
                    : ' tomorrow'
                  ].join('')
    };
...
notifier.register({
  type: 'reminder'
, id: 'gb-order-payment-status-changed'
, name: 'Order Payment Status is Ingore'
, description: [
...
, build: function( order, logger, options, callback ){
    ...
    app.render( 'order-email/gb-order-payment-status-changed', viewOptions, function( error, html ){
      if ( error ){
        return callback( error );
      }

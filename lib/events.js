  ...
  function( order, previous ){
    ...
    order.getRestaurant( function( error ){
      ...
      if (order.attributes.restaurant.voice_phones) {
        ...
        scheduler.enqueue('send-order-notification', datetime.toString(), {
...
var deliveryServices = {
  'order:type:change':
  function ( order ) {
    this.logger.info('Order type changed', { type: order.type, oldType: order.old_type });
    ...
    // type changes.
    var cancelCourier = order.old_type === 'courier' &&
                        order.type !== 'courier' &&
                        utils.contains( ['submitted', 'accepted'], order.status );

    if ( cancelCourier ) {
      scheduler.enqueue('send-order-notification', new Date(), {
        order_id: order.id
      , notification_id: 'delivery-service-cancellation'
      });
    }

    var notifyCourier = order.old_type !== 'courier' &&
                        order.type === 'courier' &&
                        utils.contains( ['submitted', 'accepted'], order.status );

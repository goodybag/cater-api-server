...
var registerChanged = function(notification) {
  ...
  , historyDataHTML: notification.historyDataHTML
  });
};
...
var registerSMSChanged = function (notification) {
  ...
  , requiredOptions: Array.isArray(notification.requiredOptions) ? notification.requiredOptions : null
  , options: Array.isArray(notification.options) ? notification.options : null
  , disablePriceHike: notification.to === 'restaurant'
  , build: function (order, logger, options, callback) {
      var sms = {
...
var notifications = [
  ...
  , historyDataHTML(hist) {
      return `<strong>Old Delivery Fee:</strong> ${(hist.data.options || {}).oldDeliveryFee}`;
    }
  }

, { name: 'Restaurant Delivery Fee Changed'
  , to: 'restaurant'
  , requiredOptions: ['oldDeliveryFee']
  , historyDataHTML(hist) {
      return `<strong>Old Delivery Fee:</strong> ${(hist.data.options || {}).oldDeliveryFee}`;
...
, { name: 'User Order Items Changed'
  , to: 'user'
  , options: ['changedDescription']
  , historyDataHTML(hist) {
      return `<strong>Changes:</strong> ${(hist.data.options || {}).changedDescription}`;
    }
  }

, { name: 'Restaurant Order Items Changed'
  , to: 'restaurant'
  , requiredOptions: ['changedDescription']
  , raw: true
  , historyDataHTML(hist) {
      return `<strong>Changes:</strong> ${(hist.data.options || {}).changedDescription}`;
    }
...
, { name: 'Restaurant Order Type Changed'
  , to: 'restaurant'
  }
];

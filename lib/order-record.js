...
var logger  = require('./logger').create('Order Records');
...
exports.generate = function( orderId, callback ){
  ...
  dirac.dals.orders.findOne( orderId, options, function( error, order ){
    ...
    var previousOrigin, previousDestination;
    var origin = Address( order.location || order.restaurant ).toString();
    var destination = Address( order ).toString();

    if ( original ) {
      previousOrigin = Address( original.location || original.restaurant ).toString();
      previousDestination = Address( original ).toString();
    }
    ...
    if ( origin !== previousOrigin || destination !== previousDestination ) {
      DMReq({
        ...
        logger.error('Failed to request distance matrix info', {error, order, origin, destination});
        order.distance_estimates = null;
      }).nodeify( callback );
    } else {
      order.distance_estimates = original ? original.distance_estimates : null;

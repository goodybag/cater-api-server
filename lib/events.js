  ...
  function( order, previous ){
    ...
    , 'render-email': function( stage, done ){
        app.render( 'order-email/order-submitted', view, function( error, html ){
          ...
          stage('get-restaurant', html);
        });
      }

    , 'get-restaurant': function( html, stage, done ){
        // this stage is necessary to get restaurant contact info
        order.getRestaurant( function( error, restaurant ){
          if ( error ){
            logger.events.error('Error getting restaurant for order #' + orderId);
            return done( error );
          }
          var email = {
            to:         restaurant.attributes.emails
          , from:       config.emails.orders
...
var welcomes = {
  ...
  function( user ){
    welcome.send( user, function( error ){
      if ( error ) return logger.events.error( 'Error sending welcome email!', error, user.toJSON() );
    });
  }
};

...
var logger      = require('../logger').paymentSummaryPhantom;
 ...
 *
 * @param  {Object}   p        The task object
 * @param  {Function} callback callback( error, results )
 */
var buildpms = function( p, callback ){
  if ( !pms.processing[ p.id ] ) pms.processing[ p.id ] = {};

  pms.processing[ p.id ][ p.jobId ] = true;

  // The output file
  var output = pms.getFullpmsPath( p.id );

  // The url to hit to build the output
  var input  = config.baseUrl + config.paymentSummaries.route.replace(
    ':id', p.id
  ).replace(
  ...
  phantom.run( args.concat( function( error, results ){
    if ( error ){
      delete pms.processing[ p.id ][ p.jobId ];
      return callback( error ), logger.error( 'Error building PMS PDF', error ), console.log('Error building PMS PDF', error);
    }

    // Upload the file to s3
    pms.upload( p.id, function( error ){
      delete pms.processing[ p.id ][ p.jobId ];

      if ( error ) return callback( error ), logger.error( 'S3 upload error', error ), console.log('S3 upload error', error);

      pms.emit( 'pms:generated', p.id );
      pms.emit( [ 'pms', p.id, 'generated' ].join(':') );
      venter.emit( 'pms:generated', p.id );
 ...
 */
pms.build = function( pmsId, rid, callback ){
  ...
  queue.push( { id: pmsId, restaurantId: rid, jobId: ++currJobId }, callback );

...
, isAvailable: function( order ){
    return (
      ...
      order.deliveryService.order_submitted_notification_id === 'dropoff-order-submitted'
    );
  }
...
, build: function( order, options, logger, callback ){
    logger = logger.create('DropoffOrderSubmitted');

    if ( !order.lat_lng ){
      logger.warn('Order was missing `lat_lng`', {
        order_id: order.id
      });
    ...
    if ( !order.location.lat_lng ){
      logger.warn('Order.location was missing `lat_lng`', {
        order_id: order.id
...
var orderDropoffOrigin = module.exports.orderDropoffOrigin = function( order ){
  ...
  origin.email = config.emails.couriers;
  origin.last_name = '#' + order.id;

  var remarks = orderRemarks( order, true );
...
var orderRemarks = module.exports.orderRemarks = function( order, includeHeadCount ){
  var remarks = [];

  if ( includeHeadCount ){
    remarks.push( 'Head Count:\n' + order.guests );
  }

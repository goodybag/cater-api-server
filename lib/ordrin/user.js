...
module.exports.register = utils.overload({
  'Number,Function':
  /**
   * Registers a user with OrdrIn
   * @param  {Number}   user_id  GB ID of user
...
, 'Object,Function':
  /**
   * Registers a user with Ordrin
   * @param  {Object}   user     GB User Object
   * @param  {Function} callback callback( error, user )
   */
...
module.exports.renewToken = utils.overload({
  'Number,Function':
  /**
   * Renews the login token for the OrdrIn User
   * @param  {Number}   user_id  GB User ID
   * @param  {Function} callback callback( error, user )
   */
  function( user_id, callback ){
    db.users.findOne( user_id, function( error, user ){
